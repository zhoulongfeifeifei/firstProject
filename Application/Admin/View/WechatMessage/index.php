<form id="pagerForm" method="post" action="/admin/WechatMessage/index">
	<input type="hidden" name="pageNum" value="1" />
	<input type="hidden" name="numPerPage" value="<?php echo $per_page; ?>" />
</form>
<div class="pageHeader">
	<form rel="pagerForm" onsubmit="return navTabSearch(this);" action="/admin/WechatMessage/index" method="post">
	<div class="searchBar">
			...
			<tr target="sid_user" rel="<?php echo $item['id'];?>">
				...
				<?php 
					switch($item['type']){
						case 1:
							$str= '文本';
							$content = replaceEmoji($item['content']);
						 break;
						 case 2:
							$str= '图片';
							$content = '<img src="'.trim($item['medialoc']).'" height="180px"/>';
						 break;
						 case 3:
							$str= '语音';
							$content = '<audio controls="controls"><source src="'.$item['medialoc'].'" />无法播放</audio>';
							 break;
						 case 4:
							$str= '视频';
							$content = '<video src="'.$item['medialoc'].'" controls="controls" width="240px">加载失败或者资源错误</video>';
							 break;
						 case 5:
							$str= '小视频';
							$content = '<video src="'.$item['medialoc'].'" controls="controls" width="240px">加载失败或者资源错误</video>';
						 ...
						 case 6:
							$str= '地理位置';
							$content = '地址：'.$item['content'];
						 break;
						 case 7:
							$str= '链接';
							$content = '<a href="'.$item['content'].'" target="_blank"><p sytle="font-size:14px;line-height:20px;">'.$item['Title'].'</p><p>'.$item['Description'].'</p></a>';
						 break;
					}
				...
				<td class="center" nowrap="nowrap">
					...
					<a title="删除" style="color:blue;" target="ajaxTodo" class="btnDel" href="/admin/WechatMessage/del_receive_msg?id=<?php echo $item['id'];?>">删除</a>
					<?php }?>
				</td>
				-->
			</tr>
