...
<div class="pageContent">
	...
	<form method="post" action="/admin/WechatMaterial/edit_material?id=<?php echo $info['id'];?>" class="pageForm required-validate" enctype="multipart/form-data" onsubmit="return iframeCallback(this, dialogAjaxDone);">

		<div class="pageFormContent" layoutH="42">
			<div class="unit">
				<label>素材类型：</label>
				<select name="type" class="type">
  					<option value="1" <?php echo $info['type']==1 ? 'selected' : '';?> >文本</option>
  					<option value="2" <?php echo $info['type']==2 ? 'selected' : '';?> >图片</option>
  					<option value="3" <?php echo $info['type']==3 ? 'selected' : '';?>>语音</option>
  					<option value="4" <?php echo $info['type']==4 ? 'selected' : '';?>>视频</option>
  					<option value="5" <?php echo $info['type']==5 ? 'selected' : '';?>>音乐</option>
  					<option value="6" <?php echo $info['type']==6 ? 'selected' : '';?>>图文</option>
   				</select>
			</div>
			<div class="unit">
				<label>名称：</label>
				<input type="text"  name="name" size="25" minlength="1" placeholder="请输入名称" maxlength="40" class="required" value="<?php echo $info['name'];?>"/>
			</div>
			<div class="unit other_show">
				<label>标题：</label>
				<input type="text"  name="title" size="25" minlength="1" placeholder="请输入标题" maxlength="100" value="<?php echo $info['title'];?>"/>
			</div>
			<div class="unit other_show">
				<label>描述：</label>
				<textarea name="description" cols="50" rows="4" placeholder="请输入描述"><?php echo $info['description'];?></textarea>>
			</div>
			<div class="unit add_img">
				...
				<dd>
					<div style="margin-left:2px;">
						...
						<?php 
							if($info['type']==2){
								echo '<img src="'.trim($info['contenturl'],'.').'" width="220" /><input type="hidden" name="media" value="'.$info['contenturl'].'" />';
							}else{
								if($info['type']==3){
									echo '<audio controls="controls"><source src="'.trim($info['contenturl'],'.').'" />无法播放</audio>';
								}else if($info['type']==4){
									echo '<video src="'.trim($info['contenturl'],'.').'" controls="controls" width="240px">加载失败或者资源错误</video>';
								}else if($info['type']==5){
									echo '<p class="music">'.$info['title'].'</p><p>'.$info['description'].'</p><p>&nbsp</p><audio controls="controls"><source src="'.trim($info['contenturl'],'.').'" />无法播放</audio>';
								}
								echo '<input type="hidden" readonly name="media" value="'.$info['contenturl'].'" />';
							}
			...
			<div class="unit img_text">
				...
				<input type="text"  name="contenturl" size="40" minlength="1" placeholder="请输入链接" maxlength="200" value="<?php echo $info['contenturl'];?>"/>
			</div>
			<div class="unit other_show">
				<label id="other_link">链接</label>
				<input type="text"  name="url" size="40" minlength="1" placeholder="请输入链接" maxlength="200" value="<?php echo $info['url'];?>"/>
			</div>
			<div class="unit text_show">
				<label>内容：</label>
				<textarea name="content" cols="50" rows="4" placeholder="请输入文本"><?php echo $info['content']?></textarea>
			</div>
			<div class="unit">
				<label>状态：</label>
...
<script type="text/javascript">
	...
	var contenturl = '<?php echo $info['contenturl'];?>';
	...
	function change_show(value){
		if(value==1){
			...
			if(value==6){
				...
				$('#imgUrlType').show();
				var imgurl = '&nbsp&nbsp&nbsp&nbsp<img id="chooseimg" src="'+contenturl+'"  height="60px;"/>';
	...
	$('.imgTypeSel').change(function(){
		...
		if(val==1){
			...
			$('.img_text').find('input').val(contenturl);
			$('#chooseimg').attr('src',contenturl);
		}else{
