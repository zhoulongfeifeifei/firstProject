...
<div class="pageContent">
	...
	<form method="post" action="/admin/Agency/edit_agency_course?ag_id=<?php echo I('get.ag_id');?>&id=<?php echo $info['id'];?>" class="pageForm required-validate" enctype="multipart/form-data" onsubmit="return iframeCallback(this, dialogAjaxDone);">
		...
		<div class="pageFormContent" layoutH="42">
			<div class="unit">
				...
				<select name="course_id">
					...
					<?php foreach($course_lists as $v){?>
						<option  <?php echo $v['sort']==1 ? '' : 'disabled';?> value="<?php echo $v['id'];?>" <?php echo $v['id']==$info['course_id'] ? 'selected' : '';?>><?php echo $v['html'].$v['name'];?></option>
					<?php }?>
				</select>
			</div>
			<div class="unit">
			...
			<div class="unit">
				<label>教室对应人数：</label>
				<input type="text" name="nums" size="16" value="<?php echo $info['nums'];?>" minlength="1" placeholder="请输入人数 " maxlength="3" class="required number" />    
			</div>
			<div class="unit">
				<label>课程时长：</label>
