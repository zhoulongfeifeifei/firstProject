<?php if (!defined('THINK_PATH')) exit();?><form id="pagerForm" method="post" action="/admin/Course">
	<input type="hidden" name="pageNum" value="1" />
	<input type="hidden" name="numPerPage" value="<?php echo $per_page; ?>" />
</form>
<div class="pageContent">
	<?php if(checkPermission('add_course')){?>
	<div class="panelBar">
		<ul class="toolBar">
			<li><a class="add" href="/admin/Course/add_course" target="dialog" width="600" height="300"mask="true" maxable="false" ><span>新建课程</span></a></li>
		</ul>
	</div>
	<div layoutH="55">
	<?php }else{?>
	...
	<table class="table" width="100%">
		<thead>
			<tr>
				...
				<th class="center" width="15%">上级课程</th>
		...
		<tbody>
			...
			<tr target="sid_user" rel="<?php echo $item['id'];?>">
				...
				<td class="center">
					<?php if($item['fid']==0){?>
					<img src="<?php echo $item['img']?>" width="26px;"/>
					<?php }else{?>
						二级课程
					<?php }?>
				</td>
				<td class="center"><?php echo $item['name'];?></td>
				<td class="center"><?php echo $item['fname'];?></td>
				...
				<td class="center" nowrap="nowrap">
					<?php if(checkPermission('edit_course')){?>
					<a title="编辑" style="color: blue;" target="dialog" mask="true" class="btnEdit" rel="edit_course"  width="600" height="300" href="/admin/Course/edit_course?id=<?php echo $item['id'];?>">编辑</a>&nbsp&nbsp
					<?php }?>
					<?php if(checkPermission('del_course')){?>
					<a title="删除" style="color:blue;" target="ajaxTodo" class="btnDel" href="/admin/Course/del_course?id=<?php echo $item['id'];?>">删除</a>
					<?php }?>
				</td>
			</tr>
