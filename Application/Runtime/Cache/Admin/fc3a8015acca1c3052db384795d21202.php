<?php if (!defined('THINK_PATH')) exit();?><form id="pagerForm" method="post" action="/WechatMessage/wechat_keywords">
	<input type="hidden" name="pageNum" value="1" />
	<input type="hidden" name="numPerPage" value="<?php echo $per_page; ?>" />
</form>
<div class="pageHeader">
	<form rel="pagerForm" onsubmit="return navTabSearch(this);" action="/WechatMessage/wechat_keywords" method="post">
	<div class="searchBar">
	 
		<ul class="searchContent">
			<li>
				<label>关键词：</label>
				<input type="text" name="words" size="20" value="<?php echo empty(I('post.words'))? '' : I('post.words');?>"/>
			</li>
...
<div class="pageContent">
	<?php if(checkPermission('add_keywords')){?>
	<div class="panelBar">
		<ul class="toolBar">
			<li><a class="add" href="/WechatMessage/add_keywords" target="dialog" width="600" height="200"mask="true" maxable="false" ><span>添加关键词</span></a></li>
		</ul>
	</div>
	<div layoutH="65">
	<?php }else{?>
	...
	<table class="table" width="100%">
		...
		<tbody>
			...
			<tr target="sid_user" rel="<?php echo $item['id'];?>">
				...
				<td class="center"><?php echo $item['words'];?></td>
				<td class="center"><?php echo $item['status']==1 ? '可用' : '禁用';?></td>
				<td class="center"><?php echo date('Y-m-d H:i:s',$item['create_time']);?></td>
				<td class="center" nowrap="nowrap">
					<?php if(checkPermission('edit_keywords')){?>
					<a title="编辑" style="color: blue;" target="dialog" mask="true" class="btnEdit" rel="edit_keywords"  width="600" height="200" href="/WechatMessage/edit_keywords?id=<?php echo $item['id'];?>">编辑</a>&nbsp&nbsp
					<?php }?>
					<?php if(checkPermission('del_keywords')){?>
					<a title="删除" style="color:blue;" target="ajaxTodo" class="btnDel" href="/WechatMessage/del_keywords?id=<?php echo $item['id'];?>">删除</a>
					<?php }?>
				</td>
			</tr>
