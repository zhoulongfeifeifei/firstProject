<?php if (!defined('THINK_PATH')) exit();?><form id="pagerForm" method="post" action="/WechatMaterial/index">
	<input type="hidden" name="pageNum" value="1" />
	<input type="hidden" name="numPerPage" value="<?php echo $per_page; ?>" />
</form>
<div class="pageHeader">
	<form rel="pagerForm" onsubmit="return navTabSearch(this);" action="/WechatMaterial/index" method="post">
	<div class="searchBar">
...
<div class="pageContent">
	...
	<div class="panelBar">
		<ul class="toolBar">
			<li><a class="add" href="/WechatMaterial/add_material" target="dialog" width="800" height="500"mask="true" maxable="false" ><span>添加素材</span></a></li>
		</ul>
	</div>
	<div layoutH="65">
	<?php }else{?>
	...
	<table class="table" width="100%">
		...
		<tbody>
			...
			<tr target="sid_user" rel="<?php echo $item['id'];?>">
				...
				<?php  switch($item['type']){ case 1: $str= '文本'; $content = $item['content']; break; case 2: $str= '图片'; $content = '<img src="'.trim($item['contenturl'],'.').'" height="80px"/>'; break; case 3: $str= '语音'; $content = '<audio controls="controls"><source src="'.trim($item['contenturl'],'.').'" />无法播放</audio>'; break; case 4: $str= '视频'; $content = '<video src="'.trim($item['contenturl'],'.').'" controls="controls" width="240px">加载失败或者资源错误</video>'; break; case 5: $str= '音乐'; $content = '<p class="music">'.$item['title'].'</p><p>'.$item['description'].'</p><p>&nbsp</p><audio controls="controls"><source src="'.trim($item['contenturl'],'.').'" />无法播放</audio>'; break; case 6: $str= '图文'; $content = '<a href="'.$item['url'].'" target="_blank"><img src="'.$item['contenturl'].'"  height="80px"/><p class="imgText">'.$item['title'].'</p></a>'; break; } echo '<td class="center">'.$content.'</td>'; echo '<td class="center">'.$str.'</td>'; ?>
				<td class="center"><?php echo $item['status']==1 ? '可用' : '禁用';?></td>
				<td class="center"><?php echo date('Y-m-d H:i:s',$item['create_time']);?></td>
				<td class="center">
					<?php
 if($item['mediaid']){ if($item['end_time']>0){ if($item['end_time']>time()){ echo date('Y-m-d H:i:s',$item['end_time']); }else{ echo '<font color="red">已过期</font>'; } }else{ echo '永久有效'; } }else{ echo '本地素材'; } ?>
				</td>
				<td class="center" nowrap="nowrap">
					<?php if(checkPermission('edit_material')){?>
					<a title="编辑" style="color: blue;" target="dialog" mask="true" class="btnEdit" rel="edit_material"  width="800" height="500" href="/WechatMaterial/edit_material?id=<?php echo $item['id'];?>">编辑</a>&nbsp&nbsp
					<?php }?>
					<?php if(checkPermission('del_material')){?>
					<a title="删除" style="color:blue;" target="ajaxTodo" class="btnDel" href="/WechatMaterial/del_material?id=<?php echo $item['id'];?>">删除</a>
					<?php }?>
				</td>
			</tr>
