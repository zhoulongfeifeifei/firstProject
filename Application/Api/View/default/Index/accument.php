		...
		<?php foreach($apis as $item){?>
			...
			<div><?php 
				foreach($item['msg'] as $msg){
					$str = '';
					$str = '<ul>';
					$str.= '<li><font color="red">'.$msg['time'].'</font>&nbsp&nbsp<span class="show_update" code="'.$msg['code'].'">'.($msg['code']==1 ? '展开' : '隐藏').'</span></li>';
					$str.='<ul class="notice_msg '.($msg['code']==1 ? 'none' : '').'">';
					foreach($msg['content'] as $val){
						$str.='<li>'.$val.'</li>';
					}
					$str.='</ul></ul>';
					echo $str;
							...
							<?php if($value['back_eg']){ $back_eg = json_decode($value['back_eg'],true);?>
								...
								<?php echo '{'; foreach($back_eg as $k=>$v){?>
									...
									&nbsp&nbsp&nbsp&nbsp
										...
										<?php if(is_numeric($k)){echo '&nbsp';}else{
											echo $k;
											}?>
										</span>
										&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp
