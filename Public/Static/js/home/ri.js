	...
	calendar_time = today;
	calendar_time_timestampe = parseInt(Date.parse(new Date(today))/1000);
	});
		...
		if(year1==year && yue==month1+1 && day1==day){
			...
			for(var i=0;i<arr.length;i++){
				...
				if(datearr[0]==year && datearr[1]==yue && datearr[2]==day){
					...
					// $('.aboluo-aclic').click(function(){
					// 	console.log('aaa');
					// })
					// var time=$('.aboluo-aclick').attr('date')
					calendar_time=$('.aboluo-aclick').attr('date');
					calendar_time_timestampe = parseInt(Date.parse(new Date(calendar_time))/1000);
...
function setTrHeight(){
	...
	for(var i=0;i<rows.length;i++){
		rows[i].style.height=(tbodyheight/rows.length-30)+"px";
		var tds=rows[i].getElementsByTagName("td");
		for(var j=0;j<tds.length;j++){
			var a=tds[j].childNodes[0];
			a.style.width=(tds[j].offsetWidth-15)+"px";
			a.style.height=(tds[j].offsetHeight-15)+"px";
			a.style.lineHeight=(tds[j].offsetHeight-10)+"px";
		}
	}
}
//得到id对象
